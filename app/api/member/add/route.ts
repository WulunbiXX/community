...
export const POST = apiHandling(async (request: NextRequest) => {
    ...
    const { name, position, number, imageUrl } = body;
    if (!name || !position || !number || !imageUrl) {
        return Response.json(
            error("All fields are required", BUSINESS_STATUE_CODE.WARNING),
            { status: 400 },
        );
    }
    ...
    const result = await collection.insertOne({ name, position, number, imageUrl, createtime: new Date().getTime(), id: uuid4() });
    return Response.json(
